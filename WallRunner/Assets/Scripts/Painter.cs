...
public class Painter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.right, out hit, 0.6f, layerMask)) {
        ...
        if (Physics.Raycast(transform.position, -transform.forward, out hit, 0.6f, layerMask)) {
            if (hit.collider.CompareTag("Wall")) {
                if (hit.transform.childCount == 0) {
                    Instantiate(paintTexture, hit.collider.bounds.center, Quaternion.FromToRotation(Vector3.forward, hit.normal), hit.transform);
                }
